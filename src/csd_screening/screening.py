        ...
        for entry in tqdm(reader):
            try:
                ...
                for mol in entry.molecule.components:
                    ...
                    # Save MOL file
                    if new or r_factors[molcode] is None or (r_factor is not None and r_factor < r_factors[molcode]):
                        mol_path = BASE_DIR / "prescreening" / f"{molcode:06d}_prescreening.mol"
...
if __name__ == "__main__":
    ...
    prescreening_molecules()
    filter_and_update_molcodes(str(BASE_DIR / "screening_step1.csv"), str(BASE_DIR / "screening_step2.csv"))
